# -*- coding: utf-8 -*-
"""Gera prateleira-tatica + Smart Hedges + Twin Coupon."""
...
def smart_hedge_html(cfg: dict) -> str:
    ...
    put = cfg["put"]  # e.g. 110 or 100
    call = cfg["call"]  # 110
    barrier = cfg["barrier"]  # e.g. 174.35
    prazo = cfg["prazo"]
    slug = cfg["slug"]

    floor = put - 100  # +10 or 0
    cap = call - 100  # +10
    ki_var = barrier - 100
    floor_label = f"+{floor:.0f}%" if floor > 0 else "0%"
    floor_put_label = f"{fmt_br(put)}%"
    barrier_label = f"{fmt_br(barrier)}%"
    ki_var_label = f"+{fmt_br(ki_var)}"

    # points for table
    spots = [-40, -20, floor, 0, 20, 40]
    if ki_var not in spots:
        spots.append(round(ki_var, 2))
    spots.extend([ki_var + 20, 90])
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    @media (max-width: 640px) {{ .page {{ padding: 16px 14px 40px; }} .topbar {{ flex-direction: column; }} .meta-pills {{ justify-content: flex-start; }} .highlights {{ grid-template-columns: 1fr; }} .mobile-sim {{ display: none; }} .chart-touch-hint {{ display: none; }} }}
  </style>
</head>
<body>
  <div class="page">
    ...
    <header class="topbar">
      <div class="brand-block">
        <div class="logos">
          ...
          <span class="logo-co"><span class="dot">{ticker[:2]}</span> {name}</span>
        </div>
    ...
    <div class="main">
      <aside class="panel">
        ...
        <p class="legend-note"><strong>B</strong> = Compra · <strong>S</strong> = Venda<br />Payoff = ativo long + opções. Barreira observada ao longo da vida.</p>
      </aside>
  ...
  <script>
    (function () {{
      ...
      function regimeFor(x) {{
        if (x <= FLOOR) return {regime_low!r};
      ...
      if (grid && yLabels && xLabels) {{
        ...
        for (var x = Math.ceil(X_MIN / 10) * 10; x <= X_MAX; x += 10) {{
          var px = xToSvg(x);
      ...
      document.getElementById("koGap").setAttribute("y1", yToSvg(KI_VAR));
      ...
      document.getElementById("capDot").setAttribute("cx", kiX + 40);
      document.getElementById("capDot").setAttribute("cy", yToSvg(CAP));
      document.getElementById("capLabel").setAttribute("x", kiX + 48);
...
""".replace("{regime_low!r}", repr(regime_low))


# Fix: I used regime_low!r incorrectly in f-string - rebuild smarter
# Actually the replace at end handles it if I put placeholder
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .lede { margin-top: 12px; font-size: 18px; font-weight: 700; color: #fff; max-width: 40em; }
    ...
    h2 { font-size: 12px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--btg-blue); margin: 28px 0 14px; }
    h2:first-of-type { margin-top: 0; }
    .ops { list-style: none; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }
    .op-block { background: var(--card); border: 1px solid var(--line); border-radius: 6px; overflow: hidden; display: flex; flex-direction: column; transition: transform .12s ease, box-shadow .12s ease, border-color .12s; }
    .op-block:hover, .op-block:focus-within { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(11,31,58,.08); border-color: #b8c6d6; }
    .op-bar { height: 6px; }
    .op { display: flex; flex-direction: column; flex: 1; text-decoration: none; color: inherit; padding: 18px 18px 16px; outline: none; }
    .op-top { display: flex; justify-content: space-between; align-items: baseline; gap: 12px; margin-bottom: 8px; }
    .op-title { font-size: 17px; font-weight: 700; color: var(--btg); }
    .op-ticker { font-size: 12px; font-weight: 700; color: var(--link); letter-spacing: 0.04em; }
    .op-blurb { font-size: 13px; color: var(--muted); line-height: 1.4; flex: 1; }
    ...
    @media (max-width: 720px) { .ops { grid-template-columns: 1fr; } }
...
<body>
  ...
  <main class="page">
    <h2>Câmbio e renda fixa</h2>
    <ul class="ops">
      <li class="op-block">
        <div class="op-bar" style="background:var(--ptax)"></div>
        <a class="op" href="../ops/ptax-call-up-out-ko/index.html">
          <div class="op-top"><span class="op-title">Call Up and Out PTAX</span><span class="op-ticker">PTAX</span></div>
          <p class="op-blurb">Call KO 110% · participa da alta até a barreira; no KO líquido +2% (rebate 5% − preço 3%).</p>
          <div class="op-meta"><span class="pill">Câmbio</span><span class="pill">Fixing 26/10</span><span class="pill">Preço 3%</span><span class="pill">Rebate 5%</span></div>
          <div class="op-cta">Abrir material →</div>
        </a>
      </li>
      <li class="op-block">
        <div class="op-bar" style="background:var(--pacb)"></div>
        <a class="op" href="../ops/pacb11-put-hedge/index.html">
          <div class="op-top"><span class="op-title">Put ATM PACB11</span><span class="op-ticker">PACB11</span></div>
          <p class="op-blurb">Compra de put ATM para hedge de duration (NTN-Bs / PACB11). Prazo 1 ano · prêmio 2,9%.</p>
          <div class="op-meta"><span class="pill">Renda fixa / ETF</span><span class="pill">1 ano</span><span class="pill">Prêmio 2,9%</span><span class="pill">ATM</span></div>
          <div class="op-cta">Abrir material →</div>
        </a>
      </li>
    </ul>
    ...
    <ul class="ops">
      <li class="op-block">
        <div class="op-bar" style="background:var(--vale)"></div>
        <a class="op" href="../ops/smart-hedge-vale3/index.html">
          <div class="op-top"><span class="op-title">Smart Hedge Vale</span><span class="op-ticker">VALE3</span></div>
          <p class="op-blurb">Proteção 110% · strike 110% · barreira 174,35% · prazo 2 anos. Piso +10%; teto +10% se atingir a barreira.</p>
          <div class="op-meta"><span class="pill">Equity</span><span class="pill">2 anos</span><span class="pill">Prot. 110%</span><span class="pill">Barreira 174,35%</span></div>
          <div class="op-cta">Abrir material →</div>
        </a>
      </li>
      <li class="op-block">
    ...
    <ul class="ops">
      <li class="op-block">
        <div class="op-bar" style="background:var(--itub)"></div>
        <a class="op" href="../ops/twin-coupon-itub4/index.html">
          <div class="op-top"><span class="op-title">Twin Coupon Itaú</span><span class="op-ticker">ITUB4</span></div>
          <p class="op-blurb">Proteção 110% · barreira de queda 70% · strike 110% · barreira 153% · prazo 2 anos.</p>
          <div class="op-meta"><span class="pill">Equity</span><span class="pill">2 anos</span><span class="pill">Prot. 110%</span><span class="pill">Down 70% · Up 153%</span></div>
          <div class="op-cta">Abrir material →</div>
        </a>
      </li>
    </ul>
...
# The smart_hedge_html has a bug with regime_low - let me rewrite a cleaner version in a second file
